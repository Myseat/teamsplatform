...
如果你需要的应用在应用市场里面找不到，Teams 平台提供了丰富多样的能力让用户或者开发者定制自己需要的应用。本书的第四章将详细介绍开发自定义应用，这里假设你拿到了一个自定义应用的安装文件——通常是一个zip文件，然后你可以如何进行安装使用。

请下载下面这个范例应用的安装文件，它包含了一个个人应用，里面包含了一个功能，就是自动打开本书的在线版本。

{% file src="../.gitbook/assets/sampleteamsapp.zip" caption="简单的自定义应用范例" %}
